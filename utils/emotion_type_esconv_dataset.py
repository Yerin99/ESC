    ...
    def __init__(
        ...
        for ex in raw:
            ...
            for turn_index, turn in enumerate(dialog):
                ...
                for prev in dialog[:turn_index]:
                    if prev.get("speaker") == "usr":
                        if not seen_usr2:
                            seen_usr2 = True
                            if emo_line:
                                first_usr_block = f"{usr_tok}{prev['text']}{sep_tok}{emo_line}{sep_tok}"
                            else:
                ...
                if emo_line and first_usr_block:
                    reserved_suffix_len = len(tokenizer(f"{sep_tok}{emo_line}{sep_tok}", add_special_tokens=False).input_ids)
