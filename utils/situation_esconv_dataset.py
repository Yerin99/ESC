    ...
    def __init__(
        ...
        for ex in raw:
            ...
            for turn_index, turn in enumerate(dialog):
                ...
                for prev in dialog[:turn_index]:
                    if prev.get("speaker") == "usr":
                        if not seen_usr2:
                            seen_usr2 = True
                            if situ_line:
                                first_usr_block = f"{usr_tok}{prev['text']}{sep_tok}{situ_line}{sep_tok}"
                            else:
                ...
                if situ_line and first_usr_block:
                    reserved_suffix_len = len(tokenizer(f"{sep_tok}{situ_line}{sep_tok}", add_special_tokens=False).input_ids)
