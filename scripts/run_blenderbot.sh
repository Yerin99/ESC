#!/usr/bin/env bash
# run_blenderbot.sh
# Run blenderbot.py with various generation settings.
# GPU allocation
...
run_single_experiment() {
  ...
  CUDA_VISIBLE_DEVICES=$cuda_device python blenderbot.py \
    $BASE_ARGS \
    --output_dir "$out_dir" \
    $specific_args \
    >"$log_file" 2>&1
...
beam_experiments() {
  echo -e "\n\n======== BLENDERBOT BEAM SEARCH EXPERIMENTS ON GPU 1 ========"
  local beams=(1 2 3 4 5 6 7 8)
  for beam in "${beams[@]}"; do
    run_single_experiment 1 "bb90M_beam${beam}" "--num_beams ${beam}"
  done
}

topk_experiments() {
  echo -e "\n\n======== BLENDERBOT TOP-K SAMPLING EXPERIMENTS ON GPU 2 ========"
  local k_vals=(10 30 50)
  for k in "${k_vals[@]}"; do
    run_single_experiment 2 "bb90M_sample_k${k}" "--top_k ${k}"
  done
}

topp_experiments() {
  echo -e "\n\n======== BLENDERBOT TOP-P SAMPLING EXPERIMENTS ON GPU 3 ========"
  local p_vals=(0.85 0.9 0.95)
  for p in "${p_vals[@]}"; do
    run_single_experiment 3 "bb90M_sample_p${p}" "--top_p ${p}"
  done
}
...
wait
echo "All BlenderBot experiments finished."
